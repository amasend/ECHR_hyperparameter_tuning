...
parser = argparse.ArgumentParser(description='Keras test')
...
parser.add_argument('--nic_name', type=str, help='Which network interface to use for communication.',
                    default=ni.interfaces()[5])
parser.add_argument('--shared_directory', type=str, help='A directory that is accessible for all processes, '
                                                         'e.g. a NFS share.',
                    default='.\\keras_test')
parser.add_argument('--backend', help='Toggles which worker is used. '
                                      'Choose between a pytorch and a keras implementation.',
                    choices=['keras'], default='keras')

args=parser.parse_args()

from keras_test import KerasWorker as worker
